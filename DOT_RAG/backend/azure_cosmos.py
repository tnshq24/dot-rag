...
class AzureCosmos(Utility):
    ...
    def get_cosmo_query(self, question_column, user_id, session_id, file_name=None, project_code=None):
        if not file_name or not project_code:
            cosmos_query = f"""SELECT TOP 5 c.{question_column}, c.rephrased_question, c.answer FROM chat_sessions c WHERE c.user_id = '{user_id}' AND c.session_id = '{session_id}' ORDER BY c._ts DESC"""
        else:
            cosmos_query = f"""SELECT TOP 5 c.{question_column}, c.rephrased_question, c.answer FROM chat_sessions c WHERE c.user_id = '{user_id}' AND c.session_id = '{session_id}' AND c.project_code = '{project_code}' AND c.file_name = '{file_name}' ORDER BY c._ts DESC"""
        return cosmos_query
