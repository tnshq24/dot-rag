...
from DOT_RAG.backend.azure_open_ai import AzureOpenAI
from DOT_RAG.backend.azure_document_intelligence import AzureDocumentIntelligence
...
class AzureAIService(AzureOpenAI, AzureDocumentIntelligence):
    ...
    async def __upload_documents_to_index(self, all_chunks, blob_name, batch_size: int=10):
        ...
        for i in range(0, len(all_chunks), batch_size):
            ...
            chunk_text_with_metadata = [f"Filename: {os.path.basename(chunk['filename'])}, Page: {chunk['page_number']}\nContent: {chunk['content']}" for chunk in batch_chunks]
    ...
    async def search_similar_documents(
        ...
        try:
            ...
            search_results = self.AZURE_SEARCH_CLIENT.search(
                ...
                # filter = "filename eq 'Categories/502/502_502-UCV-ProjectBiharAirtel.pdf'",  # Example filter, adjust as needed
                top=top_k,
            )
