    ...
    async def query(
        ...
        try:
            ...
            fetched_df = self.read_cosmo_table(
                question_column="question", user_id=user_id,
            ...
            try:
                ...
                is_language = await self.get_openai_response(messages=intent_messages) == "yes"
            ...
            relevant_docs = await self.search_similar_documents(
                cosmos_data[QUESTION_COL], top_k
            )
            ...
            if not is_relevant:
                ...
                try:
                    answer = await self.get_openai_response(messages=chat_model_prompt, json_object=False)
