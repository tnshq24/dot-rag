...
class RunAzureRagPipeline(AzureAIService, AzureCosmos, Prompt):
    ...
    async def query(
        ...
        try:
            ...
            fetched_df = self.read_cosmo_table(
                question_column=QUESTION_COL, user_id=user_id,
            ...
            try:
                ...
                intent_messages = self.get_intent_prompt(query=cosmos_data[QUESTION_COL])
                intent = await self.get_openai_response(messages=intent_messages, json_object=False)
            ...
            if not is_relevant:
                ...
                try:
                    answer = await self.get_openai_response(messages=chat_model_prompt, json_object=True)
    ...
    async def run(self, create_new_index: bool=False,
        ...
        if upload_to_blob:
            ...
            blob_name, blob_url = self.upload_pdf_to_blob(file_path=pdf_path, blob_name=blob_name, **blob_kwargs)
        if index_document:
            if upload_to_blob:
